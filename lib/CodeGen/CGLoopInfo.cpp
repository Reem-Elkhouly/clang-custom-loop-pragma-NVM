...
MDNode *LoopInfo::createMetadata(
  ...
  ///NVM///return createFullUnrollMetadata(Attrs, LoopProperties, HasUserTransforms);
  return createRecomputeMetadata (Attrs, LoopProperties, HasUserTransforms);
}
...
LoopAttributes::LoopAttributes(bool IsParallel)
    : IsParallel(IsParallel), VectorizeEnable(LoopAttributes::Unspecified),
      ...
      PipelineInitiationInterval(0), StringExpr("") {}///NVM2
...
void LoopInfoStack::push(BasicBlock *Header, clang::ASTContext &Ctx,
  ...
  for (const auto *Attr : Attrs) {
    ...
    unsigned ValueInt = 1;///NVM
    std::string ValueStr, ValueStrExtra;///NVM
    ...
    if (OpenCLHint) {
      ...
      ///NVM
      if (LH->getOption() == LoopHintAttr::Recompute){

	auto *ValueExpr = LH->getValue();
	auto *ValueExprExtra = LH->getValueExtra();
	ValueExpr->dump();
	ValueExprExtra->dump();
	clang::Expr::EvalResult EvalResult, EvalResultExtra;

	ValueExpr->EvaluateAsRValue(EvalResult, Ctx);
	ValueExprExtra->EvaluateAsRValue(EvalResultExtra, Ctx);

	ValueStr = EvalResult.Val.getAsString(Ctx, ValueExpr->getType());
	ValueStrExtra = EvalResultExtra.Val.getAsString(Ctx, ValueExprExtra->getType());
	// String comes like &foo1, cut the '&'
	ValueStr = ValueStr.substr(1,std::string::npos);
	ValueStrExtra = ValueStrExtra.substr(1,std::string::npos);
      }
    ...
    case LoopHintAttr::Numeric:
      ...
      ///NVM
    case LoopHintAttr::Persist:
      switch (Option) {
      case LoopHintAttr::VectorizeWidth:
      case LoopHintAttr::InterleaveCount:
      case LoopHintAttr::UnrollCount:

      case LoopHintAttr::Recompute:
	setStringExpr(ValueStr);
	catStringExpr(ValueStrExtra);
	break;//NVM
