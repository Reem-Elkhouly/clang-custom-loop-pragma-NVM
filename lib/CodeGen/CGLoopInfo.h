...
private:
  ...
  llvm::MDNode *
  createRecomputeMetadata(const LoopAttributes &Attrs,
                           llvm::ArrayRef<llvm::Metadata *> LoopProperties,
			  bool &HasUserTransforms);///NVM
...
public:
  ...
  ///NVM
  void setStringExpr(std::string S) {StagedAttrs.StringExpr = S;}
  void catStringExpr(std::string S) {StagedAttrs.StringExpr += " " +S;}
